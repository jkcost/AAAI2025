...
class YfinancePreprocessor(CustomPreprocessor):
    ...
    def make_alpha(self):
        ...
        for i in range(len(unique_ticker)):
            ...
            temp_indicator['KMID2'] = (temp_indicator['close'] - temp_indicator['open']) / (
                        temp_indicator['high'] - temp_indicator['low'] + 1e-12)
            temp_indicator['KUP'] = (temp_indicator['high'] - temp_indicator[['close', 'open']].max(axis=1)) / \
                                    temp_indicator['open']
            temp_indicator['KUP2'] = (temp_indicator['high'] - temp_indicator[['close', 'open']].max(axis=1)) / (
                        temp_indicator['high'] - temp_indicator['low'] + 1e-12)
            temp_indicator['KLOW'] = (temp_indicator[['close', 'open']].min(axis=1) - temp_indicator['low']) / \
                                     temp_indicator['open']
            temp_indicator['KLOW2'] = (temp_indicator[['close', 'open']].min(axis=1) - temp_indicator['low']) / (
                        temp_indicator['high'] - temp_indicator['low'] + 1e-12)
            temp_indicator['KSFT'] = (2 * temp_indicator['close'] - temp_indicator['high'] - temp_indicator['low']) / \
                                     temp_indicator['open']
            temp_indicator['KSFT2'] = (2 * temp_indicator['close'] - temp_indicator['high'] - temp_indicator['low']) / (
                        temp_indicator['high'] - temp_indicator['low'] + 1e-12)
            ...
            for w in windows:
                ...
                temp_indicator['STD' + str(w)] = temp_indicator['close'].rolling(window=w).std() / temp_indicator[
                    'close']

                # BETA
                # The rate of close price change in the past d days, divided by latest close price to remove unit
                # For example, price increase 10 dollar per day in the past d days, then Slope will be 10.
                temp_indicator[f'BETA{w}'] = temp_indicator['close'].rolling(window=w).apply(
                    lambda x: np.polyfit(range(w), x, 1)[0]) / temp_indicator['close']
                ...
                temp_indicator['MAX' + str(w)] = temp_indicator['close'].rolling(window=w).max() / temp_indicator[
                    'close']

                # MIN
                # The low price for past d days, divided by latest close price to remove unit
                temp_indicator['MIN' + str(w)] = temp_indicator['close'].rolling(window=w).min() / temp_indicator[
                    'close']

                # QTLU
                # Used with MIN and MAX
                # The 80% quantile of past d day's close price, divided by latest close price to remove unit
                temp_indicator['QTLU' + str(w)] = temp_indicator['close'].rolling(window=w).quantile(0.8) / \
                                                  temp_indicator['close']

                # QTLD
                # The 20% quantile of past d day's close price, divided by latest close price to remove unit
                temp_indicator['QTLD' + str(w)] = temp_indicator['close'].rolling(window=w).quantile(0.2) / \
                                                  temp_indicator['close']
                ...
                temp_indicator['RSV' + str(w)] = (temp_indicator['close'] - temp_indicator['MIN' + str(w)]) / (
                            temp_indicator['MAX' + str(w)] - temp_indicator['MIN' + str(w)] + 1e-12)
                ...
                temp_indicator['IMXD' + str(w)] = ((
                            temp_indicator['IMAX' + str(w)] - temp_indicator['IMIN' + str(w)])) / w
                ...
                temp_indicator[f'SUMP{w}'] = temp_indicator['pos_ret1'].rolling(w).sum() / (
                            temp_indicator['abs_ret1'].rolling(w).sum() + 1e-12)
    ...
    def make_alpha158(self):
        ...
        for i in range(len(unique_ticker)):
            ...
            for w in windows:
                ...
                temp_indicator['CORR' + str(w)] = temp_indicator['close'].rolling(window=w).corr(
                    np.log(temp_indicator['volume'] + 1))

                # CORD
                # The correlation between price change ratio and volume change ratio
                temp_indicator['CORD' + str(w)] = temp_indicator['close'].pct_change(periods=w).rolling(window=w).corr(
                    np.log(temp_indicator['volume'].pct_change(periods=w) + 1))
                ...
                # The total gain / the absolute total price changed
                # Similar to RSI indicator. https://www.investopedia.com/terms/r/rsi.asp
                temp_indicator[f'SUMP{w}'] = temp_indicator['close'].rolling(window=w).apply(
                    lambda x: sum(x[x > x.shift()])).fillna(0)
                temp_indicator[f'SUMP{w}'] = temp_indicator['pos_ret1'].rolling(w).sum() / (
                            temp_indicator['abs_ret1'].rolling(w).sum() + 1e-12)
                ...
                # Can be derived from SUMP by SUMN = 1 - SUMP
                # Similar to RSI indicator. https://www.investopedia.com/terms/r/rsi.asp
                temp_indicator[f'SUMN{w}'] = temp_indicator['close'].rolling(window=w).apply(
                    lambda x: sum(x[x < x.shift()])).fillna(0)
                temp_indicator[f'SUMN{w}'] = 1 - temp_indicator[f'SUMP{w}']
                ...
                temp_indicator[f"VMA{w}"] = temp_indicator["volume"].rolling(window=w).mean() / (
                            temp_indicator["volume"] + 1e-12)

                # VSTD
                # The standard deviation for volume in past d days.
                temp_indicator[f"VSTD{w}"] = temp_indicator["volume"].rolling(window=w).std() / (
                            temp_indicator["volume"] + 1e-12)

                # WVMA
                # The volume weighted price change volatility
                price_change = abs(temp_indicator["close"] / temp_indicator["close"].shift(1) - 1)
                temp_indicator[f"WVMA{w}"] = (price_change * temp_indicator["volume"]).rolling(window=w).std() / (
                            price_change * temp_indicator["volume"]).rolling(window=w).mean().add(1e-12)
                ...
                volume_change = temp_indicator["volume"] - temp_indicator["volume"].shift(1)
                temp_indicator[f"VSUMD{w}"] = (volume_change.rolling(window=w).apply(
                    lambda x: sum(x[x > 0])) - volume_change.rolling(window=w).apply(lambda x: sum(x[x < 0]))) / \
                                              (abs(volume_change).rolling(window=w).sum() + 1e-12)
