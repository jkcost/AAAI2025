...
class SpatialAttentionLayer(nn.Module):
    def __init__(self, num_nodes, in_features, in_len):
        ...
        # self.bn_w3 = nn.LayerNorm([num_nodes, in_len])
        # self.bn_w2 = nn.LayerNorm([num_nodes,in_len])
...
class SAGCN(nn.Module):
    def __init__(self, num_nodes, in_features, hidden_dim, window_len,
                 dropout=0.3, kernel_size=2, layers=4, supports=[0],
        ...
        receptive_field = 1 #what to do this variable?
        self.supports_len = 0
        ...
        if self.gcn_bool and addaptiveadj:
            if aptinit is None:
                if supports is None:
                    self.supports = []
                self.nodevec = nn.Parameter(torch.randn(num_nodes, 1), requires_grad=True) #num_nodes = args.num_assets
                self.supports_len += 1
        ...
        a_s_records = []
        dilation = 1
        for l in range(layers): #layers_num = 4
    ...
    def forward(self, X,A):
        ...
        if self.gcn_bool and self.addaptiveadj and self.supports is not None:
            ...
            adp_matrix_expanded = adp_matrix.unsqueeze(0).expand(A.shape[0],-1,-1)
            new_supports = [A] + [adp_matrix_expanded]
...
class ASU(nn.Module):
    def __init__(self, num_nodes, in_features, hidden_dim, window_len = 5,
                 dropout=0.3, kernel_size=2, layers=4, supports=[0],
                 spatial_bool=True, addaptiveadj=True, aptinit=None):
    ...
    def forward(self, inputs,A):
        """
        inputs: [batch, num_stock, num_features,window_len]
        mask: [batch, num_stock]
        outputs: [batch, scores]
        """
        # if len(inputs.shape) == 4:
        #     batch_size = inputs.shape[0]
        #     num_stock = inputs.shape[1]
        #     num_features = inputs.shape[2]
        #     window_len = inputs.shape[3]
        #     inputs =  inputs.reshape(batch_size*num_stock,num_features,window_len)

        # inputs = inputs.permute(1,0,2)
        if inputs.shape[0] == 1:
            self.sagcn.eval()
        x = self.bn1(self.sagcn(inputs,A))
